package com.smartstore.probadores.backend.rfidprototype;
        ...
        for (Tag t : tags) {
            ...
            try {
                parseSGTIN = ParseSGTIN.Builder()
                        .withRFIDTag(t.getEpc().toHexString())
                        .build();
                SGTIN sgtin = parseSGTIN.getSGTIN();
                System.out.print(" SGTIN: " + sgtin.getCompanyPrefix() + sgtin.getItemReference() + sgtin.getCheckDigit());
            } catch (Exception e) {
                e.printStackTrace();
            }
